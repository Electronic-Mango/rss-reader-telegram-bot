...
def get_feed_data_for_chat(chat_id: str) -> list[RssFeedData]:
...
def add_feed_to_db(
    ...
    chat_collection = _get_collection(chat_id)
    feed_link_data = RssFeedData(feed_name, feed_type, feed_link, latest_entry_id)
...
def get_all_rss_from_db() -> dict[str, list[RssFeedData]]:
...
# TODO Add return type
def _db_document_to_rss_data(document: Any) -> RssFeedData:
    if document is None:
        return None
    return RssFeedData(
        document["feed_name"],
