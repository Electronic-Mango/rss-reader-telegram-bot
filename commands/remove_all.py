...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info(f"User cancelled removing all subscriptions chat ID=[{update.effective_chat.id}].")
...
async def request_confirmation_1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info(f"User requested removal of all subscriptions")
...
async def request_confirmation_2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if confirmation_1 != CONFIRM_1_YES:
        logger.info("User cancelled the removal for first confirmation")
        return await cancel(update, context)
    logger.info(f"User confirmed removal of all subscriptions for the first time")
...
async def remove_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if confirmation_2 != CONFIRM_2_YES:
        logger.info("User cancelled the removal for second confirmation")
        return await cancel(update, context)
    chat_id = update.effective_chat.id
    logger.info(f"Remove all from chat ID: [{chat_id}]")
